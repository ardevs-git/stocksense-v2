...
import { TrashIcon, PlusCircleIcon, AlertTriangleIcon, PencilIcon, CheckCircleIcon } from './shared/Icons';
import Modal from './shared/Modal';
import ConfirmModal from './shared/ConfirmModal';
import { TaxRate, AdditionalCharge, Category, Vendor, Department } from '../types';
...
const ManageCompanyDetails: React.FC = () => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
                <label htmlFor="dashboardTitle" className="block text-sm font-medium text-gray-700">Dashboard Title</label>
                <input type="text" name="dashboardTitle" id="dashboardTitle" value={details.dashboardTitle || ''} onChange={handleChange} placeholder="Business Intelligence" className={`${commonInputStyle} mt-1`} />
            </div>
             <div>
        ...
        <div>
            <form onSubmit={handleAdd} className="flex gap-2 mb-4">
                <input 
                    ...
                    className={`flex-grow ${commonInputStyle}`}
                    required 
                />
                <button type="submit" className="bg-secondary text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600 shadow-sm">Add</button>
            </form>
            <div className="border rounded-lg overflow-hidden border-gray-200">
                <ul className="divide-y divide-gray-200 max-h-60 overflow-y-auto bg-white">
                    {categories.map(category => (
                        <li key={category.id} className="text-sm p-3 hover:bg-gray-50 flex justify-between items-center transition-colors">
                            {editingId === category.id ? (
                                <div className="flex gap-2 flex-grow items-center">
                                    <input 
                                        ...
                                        className={`flex-grow ${commonInputStyle} py-1.5`}
                                        autoFocus
                                    />
                                    <button onClick={saveEdit} className="text-green-600 hover:text-green-800 font-medium px-2">Save</button>
                                    <button onClick={cancelEdit} className="text-gray-500 hover:text-gray-700 px-2">Cancel</button>
                                </div>
                            ) : (
                                <>
                                    <span className="font-medium text-gray-900">{category.name}</span>
                                    <div className="flex gap-1">
                                        <button onClick={() => startEditing(category)} className="text-blue-500 hover:text-blue-700 p-1.5 rounded-full hover:bg-blue-50 transition-colors" title="Edit">
                                            <PencilIcon className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => handleDelete(category.id)} className="text-red-400 hover:text-red-600 p-1.5 rounded-full hover:bg-red-50 transition-colors" title="Delete">
                    ...
                    {categories.length === 0 && (
                        <li className="p-4 text-center text-gray-500 italic">No categories found.</li>
                    )}
                </ul>
            </div>
        </div>
...
const ManageVendors: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.name) {
            alert('Vendor Name is compulsory.');
            return;
        }
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={handleOpenAdd} className="bg-secondary text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600 shadow-sm flex items-center text-sm">
                    <PlusCircleIcon className="w-4 h-4 mr-2" /> Add Vendor
                </button>
            </div>
                    ...
                    {vendors.map(v => (
                        <li key={v.id} className="text-sm p-3 hover:bg-gray-50 flex justify-between items-center transition-colors">
                            <div className="flex flex-col">
                                ...
                                <div className="text-xs text-gray-500 flex flex-col sm:flex-row sm:gap-4 mt-0.5">
                                    {v.email && <span>{v.email}</span>}
                                    {v.phone && <span>{v.phone}</span>}
                                    {v.gstin && <span>GST: {v.gstin}</span>}
                                </div>
                            </div>
                            <div className="flex gap-1">
                                <button onClick={() => handleOpenEdit(v)} className="text-blue-500 hover:text-blue-700 p-1.5 rounded-full hover:bg-blue-50 transition-colors" title="Edit">
                                    <PencilIcon className="w-4 h-4" />
                                </button>
                                <button onClick={() => handleDelete(v.id, v.name)} className="text-red-400 hover:text-red-600 p-1.5 rounded-full hover:bg-red-50 transition-colors" title="Delete">
                                    <TrashIcon className="w-4 h-4" />
                                </button>
                            </div>
                        </li>
                    ))}
                    {vendors.length === 0 && <li className="p-4 text-center text-gray-500">No vendors found.</li>}
                </ul>
            </div>

            <Modal title={editingVendor ? "Edit Vendor" : "Add New Vendor"} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Vendor Name <span className="text-red-500">*</span></label>
                        <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required className={commonInputStyle} placeholder="Global Foods Inc." />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Phone</label>
                            <input type="text" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className={commonInputStyle} placeholder="+1 234 567 890" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Email</label>
                            <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className={commonInputStyle} placeholder="contact@vendor.com" />
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">GSTIN</label>
                        <input type="text" value={formData.gstin} onChange={e => setFormData({...formData, gstin: e.target.value})} className={commonInputStyle} placeholder="27ABCDE1234F1Z5" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Address</label>
                        <input type="text" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className={commonInputStyle} placeholder="123 Supply Street, Warehouse City" />
                    </div>
                    <div className="flex justify-end pt-4">
                        <button type="button" onClick={() => setIsModalOpen(false)} className="mr-2 bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-lg">Cancel</button>
                        <button type="submit" className="bg-primary hover:bg-primary-dark text-white font-bold py-2 px-4 rounded-lg">Save Vendor</button>
                    </div>
                </form>
            </Modal>
        </div>
...
const ManageDepartments: React.FC = () => {
    const { departments, addDepartment, updateDepartment, deleteDepartment } = useInventory();
    const [newName, setNewName] = useState('');
    const [editingId, setEditingId] = useState<number | null>(null);
    const [editingName, setEditingName] = useState('');
    ...
    const handleAdd = (e: React.FormEvent) => {
        e.preventDefault();
        if (newName.trim()) {
            addDepartment({ name: newName.trim() });
            setNewName('');
        }
    };
            ...
            <form onSubmit={handleAdd} className="flex gap-2 mb-4">
             ...
             <div className="border rounded-lg overflow-hidden border-gray-200">
                <ul className="divide-y divide-gray-200 max-h-40 overflow-y-auto bg-white">
                    {departments.map(d => (
                        <li key={d.id} className="text-sm p-3 hover:bg-gray-50 flex justify-between items-center transition-colors">
                            {editingId === d.id ? (
                                <div className="flex gap-2 flex-grow items-center">
                                    <input 
                                        type="text" 
                                ...
                                <>
                                    ...
                                    <div className="flex gap-1">
                                        <button onClick={() => startEditing(d)} className="text-blue-500 hover:text-blue-700 p-1.5 rounded-full hover:bg-blue-50 transition-colors" title="Edit">
                                            <PencilIcon className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => handleDelete(d.id)} className="text-red-400 hover:text-red-600 p-1.5 rounded-full hover:bg-red-50 transition-colors" title="Delete">
                                            <TrashIcon className="w-4 h-4" />
                                        </button>
                                    </div>
                                </>
                    ...
                    {departments.length === 0 && <li className="p-4 text-center text-gray-500">No departments found.</li>}
                </ul>
            </div>
        </div>
    );
    ...
    return (
        <>
            <div className="flex flex-col gap-6">
                ...
                <div className="flex items-start gap-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                    ...
                    <div className="flex-1">
                        <h4 className="text-lg font-bold text-red-800">Factory Reset & Clear Stock</h4>
                        <p className="text-sm text-red-700 mt-1">
                            This action will <strong>permanently delete ALL transactional data</strong>. Master data will be preserved, but their quantities will be zeroed out.
                        </p>
                        <div className="flex justify-end mt-4">
                            <button 
                                onClick={() => setIsFactoryResetConfirmOpen(true)}
                                className="bg-red-600 hover:bg-red-700 text-white font-bold py-2.5 px-6 rounded-lg shadow transition-colors flex items-center gap-2"
                            >
            ...
            <ConfirmModal
                ...
                title="Confirm Factory Reset & Clear Stock"
                message="Are you absolutely sure you want to perform a full factory reset? This action will permanently delete ALL transactional data (Purchases, Outwards, Sales, Payments) AND reset ALL product stock quantities to zero. This action CANNOT be undone."
                confirmButtonText="Yes, Proceed with Full Reset"
            />
        </>
    );
};
                ...
                {activeTab === 'general' && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                        <div className="space-y-8">
                            ...
                            <SettingsCard title="Manage Categories" description="Add, edit, or delete product categories.">
                                <ManageCategories />
                            </SettingsCard>
